(() => {
        ...
        try {
            return await new Promise((r, c) => {
                o.sendMessage({ source: e, type: t, data: a }, (e) => {
                    o.lastError ? c(new Error(`Error sending message: ${o.lastError.message}`)) : r(e);
                });
            });
        } catch (e) {
    ...
    async function r(e) {
        ...
        try {
            const { timeout: a, maxRetries: r } = t,
            ...
            if (e instanceof HTMLCanvasElement || (e.tagName && 'CANVAS' === e.tagName.toUpperCase()))
                try {
                    ...
                    const g = 'image/png',
                        m = u.toDataURL(g).split(',')[1];
                    return m || (console.log('Không thể lấy base64 từ canvas'), '');
                } catch (h) {
                    ...
                    for (let y = 0; y < r; y++)
                        try {
                            const k = new AbortController(),
                                I = setTimeout(() => k.abort(), a),
                                b = await fetch(e, { signal: k.signal });
                            if ((clearTimeout(I), !b.ok)) throw new Error(`HTTP error ${b.status}`);
                            ...
                            if (y === r - 1) return (console.log('Hết lượt thử fetch:', E), '');
                            await o(500);
                        }
                } catch (v) {
                    return (console.log('Lỗi khi fetch URL:', v), '');
                }
            }
    ...
    async function h() {
        ...
        const e = document.querySelector(i.inputSelector);
        if (!e) return;
        let a = null;
        for (let e = 0; e < 10 && ((a = document.querySelector(i.captchaImageSelector)), !a); e++) await o(1e3);
        if (!a) return;
        let c = '';
        for (let e = 0; e < 10; e++) {
            switch (n) {
                ...
                case 'en-gb.facebook.com':
                    c = await t('IMAGETOTEXT', 'createImageBase64', { url: a.src });
                    break;
        ...
        if (!c) return;
        console.log('Day la base64', c);
        const s = await (async function (e) {
            const a = JSON.stringify({ clientKey: l, task: e });
            try {
                const e = await t('IMAGETOTEXT', 'createTask', a);
                return e || (console.error('[imagetotext] No taskId returned from createTask'), '');
            } catch (e) {
                return (console.error('[imagetotext] Failed to create task:', e), '');
            }
        })({ type: 'ImageToTextTask', imageBase64: c });
        if (!s) return;
        const u = await (async function (e, a) {
            const o = JSON.stringify({ clientKey: l, taskId: e });
            try {
                const e = await t('IMAGETOTEXT', 'getTaskResult', { data: o, timeWait: a });
                return e ? ('fail' === e.status ? (console.error('[imagetotext] API error:', e), null) : e.solution ? e.solution : (console.error('[imagetotext] Invalid API response: missing solution'), null)) : (console.error('[imagetotext] No result returned from getTaskResult'), null);
            } catch (e) {
                return (console.error('[imagetotext] Failed to get task result:', e), null);
            }
        })(s, 20);
        u && ((e.value = u.text), e.dispatchEvent(new Event('input', { bubbles: !0 })), (m = !0), console.log('Captcha solved successfully, stop solving.'));
    }
    function p(e) {
        try {
    ...
    (async () => {
        await (async function () {
            ((s = await a(e.POWER_ON.key, e.POWER_ON.defaultValue)), (l = await a(e.API_KEY.key, e.API_KEY.defaultValue)));
        })();
        new MutationObserver(async (e, t) => {
            await (async function (e) {
                if (s && l && 'YOUR_CLIEN_KEY' !== l && i && !m)
                    for (const t of e)
                        if ('childList' === t.type && document.querySelector(i.captchaImageSelector) && !u) {
                            ((u = !0), await o(1500));
                            try {
                                if (!(await c([i.captchaImageSelector], 1e4))) return;
                                document.querySelector(i.captchaImageSelector) && (await h());
                            } catch (e) {
                                console.error('Lỗi captchaImageToText:', e);
                            } finally {
                                u = !1;
                            }
                        }
