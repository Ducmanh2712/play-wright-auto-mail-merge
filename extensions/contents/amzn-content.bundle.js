(() => {
        ...
        try {
            ...
            if (e instanceof HTMLCanvasElement || (e.tagName && 'CANVAS' === e.tagName.toUpperCase()))
                try {
                    ...
                    ((u.width = l), (u.height = i));
                    u.getContext('2d').drawImage(e, 0, 0, l, i);
                    const d = 'image/png',
            ...
            if (e instanceof Element)
                try {
                    ...
                    if (f && f.startsWith('data:image/')) {
                        const h = f.split(',')[1];
                        return h || (console.log('Base64 rỗng từ src của element'), '');
                    }
                    e = f || '';
                } catch (y) {
                    ...
                    for (let k = 0; k < n; k++)
                        try {
                            const p = new AbortController(),
                                b = setTimeout(() => p.abort(), o),
                                v = await fetch(e, { signal: p.signal });
                            ...
                            if (k === n - 1) return (console.log('Hết lượt thử fetch:', E), '');
                            await a(500);
                        }
                } catch (C) {
    ...
    async function u() {
        ...
        const t = await o(e.AMZN.key, e.AMZN.defaultValue);
        ((c = t.delayClick), (l = t.loop), (i = t.isActive));
    }
    ...
    async function m() {
        for (let t = 0; t < 30; t++) {
            await a(1e3);
            if (!(await f(['[id*="captcha"] [class*="amzn"] canvas'], 1e4))) return;
            ...
            if (!e) return void console.log('khong thaY');
            console.log('>>>>>>>>>', e);
            let r = '';
            for (let t = 0; t < 10; t++) {
                if (((r = await n(e, { timeout: 5e3, maxRetries: 3 })), r && g !== r)) {
                    g = r;
                    break;
                }
                await a(1e3);
            ...
            console.log(r);
            const s = await h(r, o);
            if ((console.log('Day la taskId>>>>>>>', s), s)) {
                const e = await y(s, 30);
                if (!e && l) {
                ...
                (await a(1e3), document.querySelector('[id*="amzn-btn-verify"]').click());
            } else if (l) {
                document.querySelector('[id*="amzn-btn-refresh"][type="button"]').click();
                continue;
            }
    ...
    async function h(e, o) {
        const a = JSON.stringify({ clientKey: s, task: { type: 'AmznSelectObjectTask', imageBase64: e, other: o } });
        try {
            const e = await t('AMZN', 'createTask', a);
            return e || (console.error('[amzn] No taskId returned from createTask'), '');
        } catch (e) {
            return (console.error('[amzn] Failed to create task:', e), '');
        }
    }
    async function y(e, o) {
        const a = JSON.stringify({ clientKey: s, taskId: e });
        try {
            const e = await t('AMZN', 'getTaskResult', { data: a, timeWait: o });
            return e ? ('fail' === e.status ? (console.error('[amzn] API error:', e), null) : e.solution ? e.solution : (console.error('[amzn] Invalid API response: missing solution'), null)) : (console.error('[amzn] No result returned from getTaskResult'), null);
        } catch (e) {
            return (console.error('[amzn] Failed to get task result:', e), null);
        }
    }
    (async () => {
        await u();
        new MutationObserver(async (e, t) => {
            await (async function (e) {
                if ((await u(), !r || !i || !s || 'YOUR_CLIEN_KEY' === s)) return;
                for (const t of e)
                    if ('childList' === t.type) {
                        if (document.querySelector('[id*="captcha"] [class*="amzn"] canvas') && !d) {
                            if (!s) return;
                            d = !0;
                            try {
                                (await a(1e3), await m());
